...
def compute_intrinsic_reward(buffer, state, embedding, prediction_net, random_net):
    ...
    return intrinsic_reward, error
    
def optimize_model(memory, policy_net, target_net, optimizer, error):
    ...
    loss = pow(td_error, 2) * weights.detach() #+ error
    loss = loss.mean()
...
def train(n_episodes, render=False):
    ...
    params = list(policy_net.parameters()) + list(prediction_net.parameters())
    ...
    for episode in range(n_episodes):
        ...
        for t in count():
            ...
            if t > INITIAL_BUFFER:
                intrinsic_reward, error = compute_intrinsic_reward(buffer, state, embedding, prediction_net, random_net)
                ...
                if len(memory) > INITIAL_MEMORY:
                    optimize_model(memory, policy_net, target_net, optimizer, error)
