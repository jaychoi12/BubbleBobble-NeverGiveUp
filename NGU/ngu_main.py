...
def train(n_episodes, render=False):
    ...
    memory = torch.load('saved/memory.pt')
    ...
    for episode in range(n_episodes):
        ...
        for t in count():
            ...
            if t > INITIAL_BUFFER:
                ...
                #reward = torch.tensor([reward]).to(device) + 0.3 * intrinsic_reward
                reward = torch.tensor([info['score']]).to(device) + 0.3 * intrinsic_reward
        ...
        if episode % 20 == 0:
            ...
            torch.save(memory, 'saved/memory.pt')
