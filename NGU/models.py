...
class Duel_DQN(nn.Module):
    def __init__(self, h, w, outputs):
        ...
        self.head2 = nn.Linear(linear_input_size, 1)
...
class ICM(nn.Module):
    def __init__(self, h, w, outputs):
        ...
        self.fcn1 = nn.Linear(linear_input_size, 128)
        self.fcn2 = nn.Sequential(nn.Linear(256, outputs),
                                  nn.Softmax(dim=-1))
...
class RND(nn.Module):
    ...
    def forward(self, x):
        ...
        g = F.relu(self.fcn1(x.view(x.size(0), -1)))
        return g
